...
def validate_model(
    seconds: float = typer.Option(1.0, "--seconds", min=0.2, help="Simulation length seconds"),
    rate: float = typer.Option(20.0, "--rate", help="Poisson rate Hz"),
    units: int = typer.Option(64, "--units", help="Motor units"),
    fft_threshold: int = typer.Option(
        2048,
        "--fft-threshold",
        min=32,
        help="Switch to FFT convolution once signals exceed this length.",
    ),
):
    dt = 0.001
...
def simulate(
    ...
    dt = 0.001
    out = scenario_sim(seconds=seconds, dt=dt, units=units, rate_hz=rate, fft_threshold=fft_threshold)
    # Validate
