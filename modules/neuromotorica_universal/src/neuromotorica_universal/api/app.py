...
def session_start(req: SessionStartReq)->SessionStartResp:
    sid=f"sess-{len(SESSIONS)+1}"; sess=Session(id=sid, exercise_id=req.exercise_id, dt=req.dt, nmj=NMJ(dt=req.dt))
    SESSIONS[sid]=sess; return SessionStartResp(session_id=sid)
@app.post("/v1/session/{sid}/signal")
def session_signal(sid:str, req:SignalReq):
    sess=SESSIONS.get(sid); 
    if not sess: raise HTTPException(404,"session not found")
    y=sess.nmj.step(req.u); sess.outputs.append(y); return {"y": y}
@app.get("/v1/policy/best")
def policy_best(session_id: Optional[str]=None, k:int=3):
    return {"cues": POLICY.select(k=k)}
@app.post("/v1/policy/outcome")
def policy_outcome(req:PolicyOutcomeReq): POLICY.update(req.cue_text, req.success); return {"status":"ok"}
@app.post("/v1/session/{sid}/summary", response_model=SummaryResp)
def session_summary(sid:str, req:SummaryReq)->SummaryResp:
    sess=SESSIONS.get(sid); 
    if not sess: raise HTTPException(404,"session not found")
    m=sess.nmj.metrics(); return SummaryResp(metrics=m.__dict__)
def run()->None:
