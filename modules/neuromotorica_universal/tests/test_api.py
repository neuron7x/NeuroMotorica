...
def test_full_session_flow():
    ...
    assert r.status_code==200; sid=r.json()["session_id"]
    for _ in range(50):
        rr=client.post(f"/v1/session/{sid}/signal", json={"u":1.0})
        assert rr.status_code==200 and 0.0 <= rr.json()["y"] <= 1.0
    rbest=client.get("/v1/policy/best", params={"session_id":sid,"k":3})
    assert rbest.status_code==200 and len(rbest.json()["cues"])==3
    ...
    assert rs.status_code==200 and rs.json()["metrics"]["peak_force"]>0.5
